...
# Imagen removed - using DALL-E only for image generation
IMAGEN_AVAILABLE = False
IMAGEN_CONFIGURED = False
print("🎨 Using DALL-E 3 only for image generation")
...
# Image generation now uses DALL-E only
IMAGEN_CONFIGURED = False

print("🧠 Initializing Horizon AI Assistant...")

# Enhanced conversation wisdom based on topics
CONVERSATION_WISDOM = {
    'technology': [
...
def build_conversation_prompt(user_input, personality, context):
    """Build basic conversation prompt with personality and context"""
    try:
        base_prompt = f"You are a helpful AI assistant with a {personality} personality. "
        
        if context:
            base_prompt += f"Context: {context} "
        
        base_prompt += f"Please respond to: {user_input}"
        
        return base_prompt
    except Exception as e:
        print(f"Error building conversation prompt: {e}")
        return f"Please respond to: {user_input}"

def log_error_with_context(message, error=None, context=None):
...
def handle_image_generation(text):
    ...
    try:
        # Debug logging
        print(f"🔍 IMAGE DEBUG: AI_MODEL_AVAILABLE={AI_MODEL_AVAILABLE}, client={client is not None}")
        ...
        if AI_MODEL_AVAILABLE and client:
            ...
            print("🎨 No image generation available - OpenAI client not available")
            return "🎨 I'd love to generate images for you! However, I need an OpenAI API key to access DALL-E image generation. Please check your configuration and try again."
    except Exception as e:
...
# Removed handle_imagen_generation - using DALL-E only for image generation

def handle_dalle_image_generation_with_enhancement(text, enhanced_prompt):ced_prompt)
            
            if local_url:
                # Create a full URL that opens the image directly in browser
def handle_image_generation(text):
    """Handle AI image generation requests using DALL-E API only"""
    try:
        print(f"🎨 Image generation request: {text}")
        # Use DALL-E only for image generation (simplified architecture)
        if AI_MODEL_AVAILABLE and client:
            return handle_dalle_image_generation(text)
        else:
            return "🎨 I'd love to generate images for you! However, I need an OpenAI API key to access DALL-E image generation. Please check your configuration and try again."
...
# Intent recognition
INTENT_PATTERNS = {
    # Only very specific, obvious utility commands - everything else should go to ChatGPT
    'time': [r'\bwhat time is it\b', r'\bcurrent time\b', r'\btime now\b'],
    'date': [r'\bwhat date is it\b', r'\btodays date\b', r'\bcurrent date\b', r'\bwhat day is it\b'],
    'math': [r'\d+\s*[\+\-\*×÷\/\^]\s*\d+'],  # Only obvious math expressions like "5+3"
    'timer': [r'\bset timer for \d+\b', r'\btimer for \d+ minutes\b'],
    'reminder': [r'\bremind me to .+ in \d+\b', r'\bset reminder for\b']
...
def is_quick_command(intent):
    ...
    quick_commands = ['time', 'date', 'math', 'timer', 'reminder']
    return intent in quick_commandse,
...
def handle_contextual_quick_response(intent, personality, context_data):
    ...
    # Handle generation commands directly using their dedicated handlers
    if intent == 'image_generation':
        # Get the actual user input from context if available
        if context_data and 'user_input' in context_data:
            return handle_image_generation(context_data['user_input'])
        return handle_image_generation("generate an image")
    elif intent == 'video_generation':
        if context_data and 'user_input' in context_data:
            return handle_video_generation(context_data['user_input'])
        return handle_video_generation("generate a video")
    elif intent == 'gif_generation':
        if context_data and 'user_input' in context_data:
            return handle_gif_generation(context_data['user_input'])
        return handle_gif_generation("generate a gif")
    elif intent == 'music_generation':
        if context_data and 'user_input' in context_data:
            return handle_music_generation(context_data['user_input'])
        return handle_music_generation("generate music")
    elif intent == 'voice_generation':
        if context_data and 'user_input' in context_data:
            return handle_voice_generation(context_data['user_input'])
        return handle_voice_generation("generate voice")
    elif intent == 'audio_transcription':
        if context_data and 'user_input' in context_data:
            return handle_audio_transcription(context_data['user_input'])
        return handle_audio_transcription("transcribe audio")
    elif intent == 'logo_generation':
        if context_data and 'user_input' in context_data:
            return handle_logo_generation(context_data['user_input'])
        return handle_logo_generation("generate a logo")
    
    # Default contextual response for non-generation intents
    base_response = f"I understand you're asking about {intent}. "
    ...
    return base_response + " How else can I assist you?"dio")
    elif intent == 'logo_generation':
        return handle_logo_generation("generate a logo")
    
    # Default contextual response for other intents
    base_response = f"I understand you're asking about {intent}. "
    ...
    return base_response + " How else can I assist you?"personality, context)
...
def get_contextual_ai_response(prompt, personality, context_data):
    ...
    # Use the existing process_user_input function which has the ChatGPT logic
    response, _, _, _ = process_user_input(prompt, personality)
    return response
        # Add time context
        time_ctx = context_data.get('time')
        if time_ctx:
